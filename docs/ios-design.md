...
1. 在项目根目录运行 `./build_scripts/build_ios_xray.sh`，脚本会调用 Xcode 提供的 `clang` 和 iOS SDK 将 xray-core 编译为适用于 arm64 的 `libxray-core.a`。
2. 将生成的静态库放入 Xcode 的 `Frameworks` 目录并链接，并在 `Runner-Bridging-Header.h` 引入 `bridge.h`。
3. 创建 `PacketTunnel` 扩展，`PacketTunnelProvider.swift` 中通过 FFI (`StartNodeService` / `StopNodeService`) 启动或停止 xray。
